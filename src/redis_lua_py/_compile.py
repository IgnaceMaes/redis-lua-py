...
_BUILTIN_HELPERS = {
    "__truthy": _TRUTHY_HELPER,
    "__isnil": _ISNIL_HELPER,
    "__or": _OR_HELPER,
    "__and": _AND_HELPER,
    "__errmsg": _ERRMSG_HELPER,
}
...
def helper_order(names: Iterable[str]) -> list[str]:
    ...
    wanted = set(names)
    return [helper for helper in [*_BUILTIN_HELPERS, *_HELPERS] if helper in wanted]


def helper_source(helper: str) -> str:
    """The Lua source of a helper, by name."""
    return (_BUILTIN_HELPERS.get(helper) or _HELPERS[helper]).rstrip()
...
def compile_body(func: Callable[..., Any], *, name: str | None = None) -> CompiledBody:
    ...
    flagged = (
        (compiler.needs_truthy, "__truthy"),
        (compiler.needs_isnil, "__isnil"),
        (compiler.needs_or, "__or"),
        (compiler.needs_and, "__and"),
        (compiler.needs_errmsg, "__errmsg"),
    )
    used = [helper for needed, helper in flagged if needed]

    return CompiledBody(
        name=name or func.__name__,
        body=lua.emit(prelude + statements).rstrip(),
        helpers=tuple(helper_order([*used, *compiler.helpers])),
